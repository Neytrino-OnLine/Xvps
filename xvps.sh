...
INBOUNDS='/opt/etc/1.txt'
OUTBOUNDS='/opt/etc/2.txt'
ROUTING='/opt/etc/3.txt'
OBSERVATORY='/opt/etc/4.txt'
BUTTON='/opt/etc/5.txt'
...
function filterSave
	...
	for LINE in $(cat $ROUTING);do
		if [ `echo "$LINE" | grep -c "$NAME"` -gt "0" -a "$FLAG" = "1" ];then
			...
			local BEGINING=$BEGINING$LINE'\n'
		elif [ `echo "$LINE" | grep -c '"type": "field"'` -gt "0" -a "$FLAG" = "2" ];then
			local ENDING=$ENDING$LINE'\n'
			local FLAG="3"
		elif [ `echo "$LINE" | grep -c '// '` -gt "0" -a "$FLAG" = "3" ];then
	...
	echo ""
	echo -e "\t1: Да"
	echo -e "\t0: Нет, вернуться к редактированию"
	echo ""
	read -r -p "Ваш выбор:"
	if [ "$REPLY" = "1" ];then
...
function listsGet
	...
	IN_LIST="$OUT$STRING"
	if [ "$1" = "2" ];then
		local LIST=`echo -e $OUTBOUNDS_TEMP | grep '//\|"tag"\|"listen"\|"port"'`
	else
...
function balancerSelect
	...
	local COUNT="0"
	local TAG=""
	echo -e "$OUT_LIST" | grep -v "pr|\|bl|" | awk -F"|" '{print "\t"$1": "$2"     "$4}'
	echo ""
	local LIST=`echo -e $OUT_LIST | grep -v "pr|\|bl|"`
...
function balancers
		...
		for LIST_ID in $LIST;do
			for READ_ID in $READ;do
				if [ "`echo $LIST_ID | awk -F"|" '{print $3}'`" = "$READ_ID" ];then
					echo ""
					echo $LIST_ID | awk -F"|" '{print "\t- "$2"     "$4}'
					echo ""
					read -r -p "Введите вес (в распределении) для данного подключения:" WEIGHT
					if [ -n "$WEIGHTS" ];then
	...
	if [ "$REPLY" = "1" ];then
		...
		if [ -n "$TIME" -a -z "`echo "$WEIGHT" | sed 's/[0-9]//g'`" ];then
			local TIME=$TIME$UNITS
		else
			local TIME='1m'
		fi
		if [ -z "$BAL_LIST" ];then
...
function rule
	...
	if [ "$REPLY" = "1" -o "$NUM" = "1" ];then
		...
		if [ `echo "$OUTTAG" | grep -c '"balance"'` -gt "0" -o `echo "$OUTTAG" | grep -c '"reservation"'` -gt "0" ];then
...
function routing
	...
	if [ "`echo -e $OUTBOUNDS_TEMP | grep -c '"protocol": "vless",'`" -gt "1" -a "$FLAG" = "2" ];then
		...
		echo ""
		read -n 1 -r -p "(Чтобы продолжить - нажмите любую клавишу...)" keypress
	elif [ "`cat $OUTBOUNDS | grep -c '"protocol": "vless",'`" -gt "1" -a ! "$FLAG" = "2" ];then
		balancers
		echo ""
		read -n 1 -r -p "(Чтобы продолжить - нажмите любую клавишу...)" keypress
	fi
...
function vps
	...
	if [ ! "$NUM" = "1" ];then
		...
		echo "vless://#UniVeRs-aLlY-UnIC-ueiD-eNTiFieRuUi#@some.domain:123?type=stn&security="
		echo "setting&pbk=#pUblI-CKeYpuBlICkeyPUbLickEyPubLIckEyPuli#&fp=torbrowser&sni=doma."
		echo "in&sid=#sHOrtidENtifIe#&spx=%2F&flow=some-settings#Country-WIRE_Username),"
	...
	if [ "$REPLY" = "1" -o "$NUM" = "1" ];then
		...
		if [ -z "$REPLY" ];then
			...
			local KEY=`echo "$REPLY" | awk '{sub(/^vless:\/\//,"uuid=")}1' | awk '{sub(/:/,"&port=")}1'| awk '{sub(/@/,"&doip=")}1'| awk '{sub(/#/,"#name=")}1' | tr \/ \= | tr \&:?@# \\\n`
			...
			local SEC=`echo -e "$KEY"_O | grep "security=" | awk '{sub(/security=/,"")}1'`
...
function portRead
	...
	if [ -n "$PORT" -a -z "`echo "$REPLY" | sed 's/[0-9]//g'`" -a ! "$REPLY" = "0" ];then
		if [ `echo -e "$INBOUNDS_TEMP" | grep -c '"port": '$PORT` -gt "0" ];then
			...
			echo -e "\t2: Всё равно использовать порт $PORT"
			echo ""
			read -r -p "Ваш выбор:"
			if [ "$REPLY" = "2" ];then
				echo ""
...
function inbounds
	...
	echo "в политику Xkeen (в настройках роутера)..."
	echo ""
	echo -e "\t1: Да"
	echo -e "\t0: Нет"
	echo ""
	...
	if [ "$POLICY" = "1" ];then
		...
		echo ""
		echo -e "\tНе забудьте создать в веб-конфигураторе роутера (Интернет/Приоритеты"
		echo "подключений) политику, с названием \"Xkeen\". И добавить в неё все подключения -"
		echo "которые используются для доступа в интернет..."
	fi
	echo ""
	proxy "1"
	...
	if [ -z "$INBOUNDS_TEMP" ];then
		echo ""
		echo -e "\еОшибка: необходимо добавить хотя бы одно входящее подключение."
		echo ""
		read -n 1 -r -p "(Чтобы продолжить - нажмите любую клавишу...)" keypress
		inbounds $FLAG
		exit
...
function mainMenu
	{
	if [ `opkg list-installed | grep -c "xkeen"` -gt "0" ];then 
		local XKEEN="1"
	else
		local XKEEN="0"
	fi
...
-u)	clear
	DT=`date +"%C%y.%m.%d %T"`
	cat /tmp/mnt/keen-webdav/xvps.sh > /opt/bin/xvps.sh
	mv /opt/bin/xvps.sh /opt/bin/xvps
